...
function log(format, ...)
	LT.LogManager.V(string.format(format, ...))
end

function logw(format, ...)
	LT.LogManager.W(string.format(format, ...))
end

function loge(format, ...)
	LT.LogManager.E(string.format(format, ...))
end
...
function LuaGC()
  ...
  LT.LogManager.V("Begin gc count = {0} kb", c)
  collectgarbage("collect")
  c = collectgarbage("count")
  LT.LogManager.V("End gc count = {0} kb", c)
end
...
function DumpTable(t)
	for k,v in pairs(t) do
		if v ~= nil then
			LT.LogManager.V("Key: {0}, Value: {1}", tostring(k), tostring(v))
		else
			LT.LogManager.V("Key: {0}, Value nil", tostring(k))
		end
	end
end
...
function PrintLua(name, lib)
	...
	if (m == nil) then
		LT.LogManager.V("Lua Module {0} not exists", name)
		return
	end

	LT.LogManager.V("-----------------Dump Table {0}-----------------",name)
	if (type(m) == "table") then
		for k,v in pairs(m) do
			LT.LogManager.V("Key: {0}, Value: {1}", k, tostring(v))
		end
	end

	local meta = getmetatable(m)
	LT.LogManager.V("-----------------Dump meta {0}-----------------",name)
	...
	LT.LogManager.V("-----------------Dump meta Over-----------------")
	LT.LogManager.V("-----------------Dump Table Over-----------------")
end